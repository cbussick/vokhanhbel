...
- The plus used on create actions (`plus`, in `src/components/AddIcon.tsx`) and the audio player's
  `rotate-ccw` replay control are also from Lucide under the same ISC license and the same notice
  file.
